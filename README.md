...
面向 2D 游戏地图的本地可视化编辑器。它不只是抠图工具，而是用资源蒙版建立角色与场景之间的遮挡、障碍和可行走关系，并导出可直接交给游戏端测试的地图逻辑包。

## 主要功能

- 使用 Grounding DINO 与 SAM 2 识别地图物件，也可导入已有透明素材作为资源蒙版。
- 为资源编辑遮挡底线，拖动小人即时预览前后遮挡效果。
- 绘制独立道路区域，并自动排除资源蒙版，避免道路与障碍重复。
- 自动生成资源可走边界：线上方可走，线下方作为障碍；自动生成只补充缺失内容，不覆盖人工精调。
- 使用角色透明图的脚底接触范围验证实际移动与遮挡。
- 导出完整地图、原尺寸遮挡层、最终 `walkable.png` 和 JSON v3 配置。
- 项目、小人、图层、道路、遮挡线和可走边界均可保存并继续编辑。

- 使用入口：双击 Windows 下的 `启动工具.bat`
- 详细说明：[2D 地图遮挡与行走编辑器使用说明](docs/map-cutout-user-guide.md)
- Cursor 继续开发：[项目交接文档](docs/CURSOR_HANDOFF.md)
- 默认项目输出位置：`D:\06\剧本\地图抠图\项目输出`

本项目基于上游 Grounded SAM 2 开发；其原始模型、演示和许可证说明保留在下文。
