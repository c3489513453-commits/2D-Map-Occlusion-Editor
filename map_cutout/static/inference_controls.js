export class InferenceControls{
 ...
 async addRecognizedBox(box){
  ...
  const {job_id}=await this.api.post(`/api/maps/${mapId}/layers/${id}/segment-add`,{box});
 ...
 async paint(stroke){
  ...
  const task=this.paintChain.then(()=>this.api.post(`/api/maps/${mapId}/layers/${id}/paint`,stroke));
