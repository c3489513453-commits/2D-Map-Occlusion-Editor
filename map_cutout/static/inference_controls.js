export class InferenceControls{
 ...
 bind(){this.el.detect.addEventListener("click",()=>this.run(()=>this.detect()));this.el.generate.addEventListener("click",()=>this.run(()=>this.generate()));this.el.commit.addEventListener("click",()=>this.run(()=>this.commit()));this.el.cancel.addEventListener("click",()=>this.run(()=>this.cancel()));this.editor.addEventListener("box",e=>this.prepare({box:e.detail}));this.editor.addEventListener("box-add",e=>this.run(()=>this.addRecognizedBox(e.detail)));this.editor.addEventListener("points",e=>this.prepare({points:e.detail.points,labels:e.detail.labels}));this.editor.addEventListener("stroke",e=>this.run(()=>this.paint(e.detail)))}
 async run(work){try{await work()}catch(error){this.state.setStatus("操作失败");this.toast(error.message)}}
 async flushPendingEdits(){await this.paintChain}
 ...
 async addRecognizedBox(box){
  const id=this.state.editingLayerId;
  if(!id)throw Error("请先在右侧蒙版图层上点击「继续编辑」");
  const walkable=this.state.walkableLayers?.some(item=>item.id===id);
  const layer=(walkable?this.state.walkableLayers:this.state.layers).find(item=>item.id===id);
  ...
  await this.paintChain;
  const {job_id}=await this.api.post(walkable?`/api/maps/${mapId}/walkable/layers/${id}/segment-add`:`/api/maps/${mapId}/layers/${id}/segment-add`,{box});
  const result=await this.wait(job_id,["加载模型","识别物体","生成选区","加进当前蒙版"]);
  if(this.state.currentMapId!==mapId)return;
  if(!result.added){
   const text={occupied:"这块地方已经有蒙版了",already:"这块区域已经在当前蒙版里",empty:"这里没有识别到物体"}[result.reason]||"没有新的区域可以加上";
 ...
 async paint(stroke){
  ...
  const endpoint=walkable?`/api/maps/${mapId}/walkable/layers/${id}/paint`:`/api/maps/${mapId}/layers/${id}/paint`;
  const task=this.paintChain.then(()=>this.api.post(endpoint,stroke));
  ...
  try{
    ...
    if(this.editSerial===serial&&this.state.currentMapId===mapId){
      this.editor.dropLocalMask(id);
      await this.editor.loadMasks();
    }
